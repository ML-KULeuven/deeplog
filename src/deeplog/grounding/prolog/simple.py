...
class SimpleGrounder(PrologGrounder):
    ...
    def _prove(
        ...
        if predicate in [(",", 2), (";", 2), ("not", 1), ("true", 0)]:
            results = self._prove_logical(program, goal, builder, open_predicates)
        elif predicate in self.builtins:
            results = self._prove_builtins(goal, builder)
        elif predicate in program:
            results = self._prove_clauses(program, goal, builder, open_predicates)
        else:
    ...
    def _prove_logical(
        ...
        if predicate == ("true", 0):
            ...
            for substitution1, formula1 in self._prove(
                program, lhs, builder, open_predicates
            ):
            ...
            lhs, rhs = goal[1:]
            yield from self._prove(program, lhs, builder, open_predicates)
            yield from self._prove(program, rhs, builder, open_predicates)
        elif predicate == ("not", 1):
            subgoal = cast(Symbol, goal[1])
            proof = reduce(
                builder.disjoin,
                (f for _, f in self._prove(program, subgoal, builder, open_predicates)),
                builder.get_false(),
            )
        ...
        for clause in program[get_predicate(goal)]:
            ...
            if is_fact(clause):
                ...
                for sub, f in self._prove(
                    program, apply_substitution(body, mgu), builder, open_predicates
                ):
                    if is_open:
